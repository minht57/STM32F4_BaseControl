...
void Encoder_CalVelocity(TARGET_VELOCITY_PARAMETERS * TargetVelocity)
{
  ...
  TargetVelocity->i16_Pulse_Left = (int16_t)(TargetVelocity->ratio * f_AngleVelocityLeft * PULSE_ENCODER_PER_ROUND / (2 * PI));
  TargetVelocity->i16_Pulse_Right = (int16_t)(TargetVelocity->ratio * f_AngleVelocityRight * PULSE_ENCODER_PER_ROUND / (2 * PI));
}
...
//  ResultVelocity->linear_velocity = (f_AngleVelocityLeft + f_AngleVelocityRight) * ResultVelocity->radius / 2;
  ...
  f_RatioAngleVelocity = (2 * PI * ResultVelocity->radius)/(ResultVelocity->ratio * PULSE_ENCODER_PER_ROUND * ResultVelocity->length);
  f_RatioLinearVelocity = (2 * PI * ResultVelocity->radius)/(ResultVelocity->ratio * PULSE_ENCODER_PER_ROUND * 2);
